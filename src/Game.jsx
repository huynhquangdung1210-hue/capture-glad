/**
 * Game.jsx - Main game engine for Capture-the-Glad
 * 
 * This file contains the complete game simulation including:
 * - Entity management (predators, prey, stars)
 * - Physics simulation with fixed timestep
 * - Collision detection and resolution
 * - AI integration (heuristics and RL)
 * - Canvas rendering
 * - Replay recording/playback system
 * - UI controls and HUD
 * 
 * Game Mechanics:
 * - Predators (black squares): Chase and eat prey to survive
 * - Prey (blue/orange arrows): Collect stars, avoid predators, can reproduce
 * - Stars (yellow circles): Food source for prey
 * - Arena: Circular boundary with configurable radius
 * - Special Ability: Prey can become invincible and kill nearby predators
 * - Energy System: Prey must eat stars or they starve
 * - Reproduction: Both predators and prey can reproduce when conditions are met
 */
...
export const DEFAULTS = {
  // Canvas and arena dimensions
  CANVAS_SIZE: 700,              // Canvas width/height in pixels
  ARENA_RADIUS: 300,             // Circular arena radius
  
  // Initial entity counts
  INITIAL_PREDATORS: 2,          // Starting predator count
  INITIAL_PREY: 6,               // Starting prey count
  STAR_COUNT: 15,                // Number of food stars in arena
  
  // Spawn rates
  SPECIAL_PREY_SPAWN_PER_SEC: 0.5,  // Rate of special prey spawning
  
  // Movement speeds (multiplied by timestep)
  PREDATOR_SPEED: 1.8,           // Predator movement speed
  PREY_SPEED: 2.2,               // Prey movement speed (slightly faster)
  
  // Reproduction thresholds
  PREDATOR_REPRO_EAT: 5,         // Predators reproduce after eating 5 prey
  
  // Simulation timing
  FIXED_TIMESTEP: 1 / 60,        // 60 ticks per second (physics update rate)
  PREDATOR_HUNGER_SECS: 5,       // Predators must eat every 5 seconds or die
  MAX_EPISODE_TIME: 300,         // Maximum episode duration (5 minutes)
  
  // Progressive difficulty scaling
  PREDATOR_SPAWN_INTERVAL: 10,   // Seconds between predator spawns
  PREDATOR_SPAWN_RAMP: 0.95,     // Reduce interval by 5% each spawn (difficulty ramp)
};
...
/* ================================================
   AI MODULE
   ================================================
   Simple heuristic AI for predators. Prey AI is handled
   by the preyAI.jsx module which supports both heuristics
   and reinforcement learning.
 */

function createAI() {
  /**
   * Compute predator action: chase nearest prey or wander.
   * 
   * Predators use a simple greedy strategy:
   * - If prey exist: move towards the nearest one
   * - If no prey: wander randomly
   * 
   * @param {Object} pred - Predator entity with x, y position
   * @param {Array} preyList - List of prey entities
   * @returns {Object} Action with dx, dy direction components
   */
  function predatorAction(pred, preyList) {
    // No prey available: wander randomly
...
/* ================================================
   RENDERER MODULE
   ================================================
   Handles all canvas drawing operations. Uses world coordinates
   (centered at origin) and converts to canvas coordinates for rendering.
 */

function createRenderer(ctx, canvasSize, arenaRadius) {
  const center = canvasSize / 2;  // Canvas center point
  ...
  function drawArena() {
    ctx.save();
    ctx.translate(center, center);  // Move origin to canvas center
    ctx.strokeStyle = "#444";
    ctx.lineWidth = 3;
    ctx.beginPath();
   ...
   * World origin (0,0) maps to canvas center.
   * 
...
export default function Game() {
  ...
  /* ================================================
     ...
     ================================================
     This is the authoritative simulation step that advances the game
     by dt seconds. It handles:
     - AI decision making (predators and prey)
     - Movement and physics
     - Collision detection
     - Energy/starvation mechanics
     - Special abilities
     - Reproduction
     - Entity spawning/despawning
     
     Can be called with custom actions for RL training or testing.
  */

  function step(dt) {
    const physics = physicsRef.current;
    const ai = aiRef.current;
    if (!ai) return;  // Wait for AI module to initialize

    // External actions override for RL agents or testing
    // Format: { entityId: { dx, dy } }
    ...
    for (let j = preys.length - 1; j >= 0; j--) {
      ...
      physics.keepInside(pr);
    }
    // ================================================
    // PREY SPECIAL ABILITY PROCESSING
    // ================================================
    // Process prey special abilities before predator collisions
    // so that special kills take effect immediately.
    
    const predEatRange = 12;  // Distance at which predators catch prey
    const predatorsToRemove = new Set();  // Track predators killed by special
    const specialKillRadius = PREY_STATS.specialkillradius;

    // Check each prey with active special ability
    for (const pr of preyRef.current) {
      if (pr.special > 0) {
        // Kill all predators within special ability radius
        ...
        // Reward for killing predators
        pr.rlReward += RL_WEIGHTS.specialeff * predatorsToRemove.size;
        
        // Penalty for wasting special ability (no kills this tick)
    ...
    // ================================================
    // PREDATOR-PREY COLLISIONS
    // ================================================
    // Predators catch and eat prey that come too close.
    // This resets the predator's hunger timer and may trigger reproduction.
    ...
    for (let i = predatorsRef.current.length - 1; i >= 0; i--) {
      ...
      for (let j = preyRef.current.length - 1; j >= 0; j--) {
        ...
        // Check if predator is close enough to catch prey
        if (length(P.x - pr.x, P.y - pr.y) < predEatRange) {
          // Mark prey as dead (will be removed in next cleanup)
          pr.alive = false;
          
          // Reset predator hunger timer
          P.hunger = DEFAULTS.PREDATOR_HUNGER_SECS;
          
          // Increment predator's eat counter
          P.eatenCount = (P.eatenCount || 0) + 1;
          
          // Reproduce if eaten enough prey
          if (P.eatenCount >= predatorReproCount) {
            predatorsRef.current.push(physics.reproducePredator(P));
            P.eatenCount = 0;  // Reset counter
          }
          
          break;  // Prey is eaten, move to next predator
        }
      }
    }
