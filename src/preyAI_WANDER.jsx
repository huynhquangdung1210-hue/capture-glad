...
export function getRLState(prey, stars, predators) {
  ...
  const dx_food_2 = thirdfood ? (thirdfood.x - prey.x) : 0;
  const dy_food_2 = thirdfood ? (thirdfood.y - prey.y) : 0;
  const dx_center= 0-prey.x;
  const dy_center= 0-prey.y;
  // ---------------------------
...
function rlPrey(prey, stars, predators, getRLState) {
  ...
  if (action) {
    ...
    console.log("Using RL action for prey:", action);
    return {
      dx: action.dx,
      dy: action.dy,
...
--------------------------------------*/
export function registerRLAction(preyId, dx, dy, cooldown, action_idx, moveAction, foodAction) {
  rlActions.set(preyId, { dx, dy, cooldown, action_idx, moveAction, foodAction });
